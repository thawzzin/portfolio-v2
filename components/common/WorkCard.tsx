...
const WorkCard = ({ work }: { work: any }) => {
  return (
    <div className="md:flex gap-10 border-[#D1D1C7]/30 last:border-none border-b-[1px] py-10 md:py-20">
      <Stack className={`md:w-[50%] mb-10 md:mb-0 `}>
        ...
        <Reveal>
          <div className="flex md:gap-20 justify-between md:justify-start flex-wrap">
            {work.githubUrl && (
              <ArrowButton url={work.githubUrl}>View on Github</ArrowButton>
            )}
            {work.url && (
              <ArrowButton url={work.url}>View Live Demo</ArrowButton>
            )}
          </div>
        </Reveal>
      </Stack>
