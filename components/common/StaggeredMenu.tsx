...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div
      className={`sm-scope z-40 ${open ? "" : "is-closed"} ${
        isFixed
          ? "fixed top-0 -right-0 md:right-0  w-screen h-screen overflow-hidden"
          : "w-full h-screen"
      }`}
    >
      <div
      ...
      >
        ...
        >
          ...
          <button
            ref={toggleBtnRef}
            className={`z-50 sm-toggle relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto ${
          ...
          >
            ...
            >
              <span
                ref={plusHRef}
                className="hidden sm-icon-line absolute left-1/2 top-1/2 w-full h-[2px] bg-current rounded-[2px] -translate-x-1/2 -translate-y-1/2 [will-change:transform]"
              />
              <span
                ref={plusVRef}
                className="hidden sm-icon-line sm-icon-line-v absolute left-1/2 top-1/2 w-full h-[2px] bg-current rounded-[2px] -translate-x-1/2 -translate-y-1/2 [will-change:transform]"
              />
            </span>
            <Checkbox />
          </button>
...
.sm-scope.is-closed {
  /* This is the KEY fix: It makes the full-screen div transparent to mouse clicks. */
  pointer-events: none;
  /* We remove the z-index: -1. The default z-index: 1 is fine, 
     as pointer-events: none prevents it from capturing clicks. */
}

/* The menu button is a child of the header, which is *inside* sm-scope.
   You must override the inherited 'pointer-events: none' for the button.
*/
.sm-scope.is-closed .staggered-menu-header {
  /* Set the header's pointer events to auto to re-enable interaction with the button */
  pointer-events: auto;
}

/* The toggle button itself should also explicitly be set to auto for safety */
.sm-scope.is-closed .sm-toggle {
  pointer-events: auto;
}
      `}</style>
    </div>
